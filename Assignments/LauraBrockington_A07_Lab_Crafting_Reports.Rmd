...
: Summary of metadata for the EPA Air Quality data
...
kable(summary_PM25, col.names = c("County", "$\\mu g/m^3$"),  caption = "Mean PM2.5 concentration for each county")

kable(summary_Ozone, col.names = c("County", "$\\mu g/m^3$"), caption = "Mean ozone concentration for each county")
```
...
```{r 2018, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "left", fig.height=3.5, fig.width=8, fig.cap="Distribution of ozone levels by month in 2018"}
epa_2018 <- filter(epa_data, Year == "2018")
ggplot(data = epa_2018, aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone)) +
  geom_boxplot(col = "cyan4") +
...
```{r 2019, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "left", fig.height=3.5, fig.width=8, fig.cap="Distribution of ozone levels by month in 2019"}
epa_2019 <- filter(epa_data, Year == "2019")
ggplot(data = epa_2019, aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone)) +
  geom_boxplot(col = "cyan4") +
  scale_x_discrete(drop = F, 
                    name = "Month") +
  labs(
...
My git repository is linked [HERE](https://github.com/lab165/EDA)
