...
```{r}
#Exercise 1:
density_plot <- ggplot(EPAair, aes(x = Ozone, fill = factor(Year))) +
  geom_density(alpha=0.5) +
  scale_fill_manual(values=c("darkgreen", "darkblue")) +
  labs(x="Ozone AQI Value", y="Density", fill="")
print(density_plot)

boxplot <- ggplot(EPAair, aes(y=Ozone, x=factor(Year))) +
  geom_boxplot() +
  labs(x="Year", y="Ozone AQI Value") +
  stat_summary(geom="text", fun=max, vjust=-1, size=4, label=c("b", "a"))
print(boxplot)

violin_plot <- ggplot(EPAair, aes(y=Ozone, x=factor(Year))) +
...
scatterplot <- ggplot(EPAair, aes(x=Ozone, y=(PM2.5))) +
  geom_point() +
  labs(y="PM2.5", x="Ozone AQI Value") +
...
aic <- lm(data = EPAair, PM2.5 ~ Ozone + mean_Lat + 
              mean_Lng + Month + Year + COUNTY + Site.Name)
step(aic)
