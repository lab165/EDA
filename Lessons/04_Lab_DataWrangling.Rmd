...
NTL.phys.data.Tuesday.90.99 <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))
```
...
NTL.phys.data.Ex5 <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irrandiance_Value")

#Exercise 6: Spread temperatureC into more than one column based on the depth.
NTL.phys.data.Ex6 <- pivot_wider(NTL.phys.data, names_from = depth, values_from = temperature_C)
```
